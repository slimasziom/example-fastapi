...
def get_posts(db: Session = Depends(get_db), 
    ...
    results = db.query(models.Post, func.count(models.Vote.post_uuid).label("votes")).join(
        models.Vote, models.Vote.post_uuid==models.Post.uuid, isouter=True).group_by(models.Post.uuid).filter(
...
def post_create_posts(
    ...
    new_post = models.Post(owner_uuid=current_user.uuid, **post.model_dump())
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
...
def get_post(id: int, response: Response, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_uuid).label("votes")).join(
        models.Vote, models.Vote.post_uuid==models.Post.uuid, isouter=True).group_by(models.Post.uuid).filter(models.Post.uuid == id).first()
